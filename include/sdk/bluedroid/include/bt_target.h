...
#define BTA_AG_SCO_PKT_TYPES \
    ( BTM_SCO_LINK_ONLY_MASK | BTM_SCO_PKT_TYPES_MASK_EV3 | BTM_SCO_PKT_TYPES_MASK_NO_3_EV3 | \
...
#define HCI_GET_CMD_BUF( paramlen ) \
    ( (BT_HDR *)GKI_getbuf( (UINT16)( BT_HDR_SIZE + HCIC_PREAMBLE_SIZE + ( paramlen ) ) ) )
#endif
...
/* The maximum age of entries in inquiry database based on inquiry response failure ('0' disables
 * feature). */
...
/* The L2CAP MPS over Bluetooth; must be in accord with the FCR tx pool size and ACL down buffer
 * size. */
...
/* If host flow control enabled, this is the number of buffers the controller can have
 * unacknowledged. */
#ifndef L2CAP_HOST_FC_ACL_BUFS
#define L2CAP_HOST_FC_ACL_BUFS 20
#endif

/* The percentage of the queue size allowed before a congestion event is sent to the L2CAP client
 * (typically 120%). */
...
/* Broadcom specific acknowledgement message to ensure fast and robust operation of WUG FIMA
 * procedure. */
...
/* The timeout value (in seconds) for reliable sessions to remain in suspend. 0xFFFFFFFF for no
 * timeout event. */
...
Note:  This pool needs to have enough buffers to hold two times the window size negotiated
 ...
 L2CA_SetFCROptions plus BT_HDR (8) + HCI preamble (4) + L2CAP_MIN_OFFSET (11 - as of BT 2.1 + EDR
Spec).
*/
...
/* When ACL link between TL and GW is idle for more than this amount of seconds, the ACL may be put
 * to low power mode. */
...
/* CTP TL would try to re-establish L2CAP channel after channel is disconnected for this amount of
 * seconds. */
...
/* When TL does not create another L2CAP channel within this period of time GW declares that it's
 * "Connected Limited". */
#ifndef CTP_TL_CONN_TIMEOUT
#define CTP_TL_CONN_TIMEOUT 5
#endif

/* The delay for ACL to completely disconnect (for intercom) before sending the connect request to
 * GW. */
...
#define BPP_DOC_FORMAT_SUPPORTED
"application/vnd.pwg-xhtml-print:1.0,application/vnd.hp-PCL:5E,application/PDF" #endif
...
{0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x01} #endif
 */
/* XHTML formats.
...
#ifndef BPP_1284ID
#define BPP_1284ID                  "\x00\x48MANUFACTURER:ACME Manufacturing;COMMAND
SET:PCL,MPL;MODEL:LaserBeam \?;" #endif
...
/* minimum 4, maximum is 255. The value should be set to the maximum size of encoding string + 1.
 * JPEG2000. If vendor specific format is supported, it might be bigger than 9 */
...
/* Number of seconds between subsequent credit requests to the server when the send watermark has
 * been exceeded. */
...
Note:  This pool needs to have enough buffers to hold two times the window size negotiated
 ...
 The size of each buffer must be able to hold the maximum MPS segment size passed in
 tL2CAP_FCR_OPTIONS plus BT_HDR (8) + HCI preamble (4) + L2CAP_MIN_OFFSET (11 - as of BT 2.1 + EDR
Spec).
*/
...
#define BTA_AG_CIND_INFO \
    "(\"call\",(0,1)),(\"callsetup\",(0-3)),(\"service\",(0-1)),(\"signal\",(0-5)),(\"roam\",(0," \
    "1)),(\"battchg\",(0-5)),(\"callheld\",(0-2))"
#endif
