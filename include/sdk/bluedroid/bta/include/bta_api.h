...
#include "sdk/bluedroid/stack/include/uipc_msg.h"
 ...
 * Service ID
 *
 * NOTES: When you add a new Service ID for BTA AND require to change the value of
 * BTA_MAX_SERVICE_ID, make sure that the correct security ID of the new service from Security
 * service definitions (btm_api.h) should be added to bta_service_id_to_btm_srv_id_lkup_tbl table in
 * bta_dm_act.c.
 */
...
#define BTA_SEC_AUTHORIZE \
    ( BTM_SEC_IN_AUTHORIZE ) /* Authorization required (only needed for out going connection )*/
...
#define BTA_DM_BLE_CONN_PARAM_UNDEF \
    BTM_BLE_CONN_PARAM_UNDEF /* use this value when a specific value not to be overwritten */
...
#define BTA_AUTH_SP_NO \
    BTM_AUTH_SP_NO /* 0 MITM Protection Not Required - Single Profile/non-bonding \
...
#define BTA_AUTH_AP_NO \
    BTM_AUTH_AP_NO /* 2 MITM Protection Not Required - All Profiles/dedicated bonding \
                     Numeric comparison with automatic accept allowed */
#define BTA_AUTH_AP_YES \
    BTM_AUTH_AP_YES /* 3 MITM Protection Required - All Profiles/dedicated bonding \
                      Use IO Capabilities to determine authentication procedure */
#define BTA_AUTH_SPGB_NO \
    BTM_AUTH_SPGB_NO /* 4 MITM Protection Not Required - Single Profiles/general bonding \
                       Numeric comparison with automatic accept allowed */
#define BTA_AUTH_SPGB_YES \
    BTM_AUTH_SPGB_YES /* 5 MITM Protection Required - Single Profiles/general bonding \
                        Use IO Capabilities to determine authentication procedure */
...
typedef struct
{
    ...
    DEV_CLASS dev_class; /* peer CoD */
    BD_NAME bd_name;     /* peer device name */
    UINT32 passkey; /* the numeric value for comparison. If just_works, do not show this number to
                       UI */
...
/* multi adv callback */
typedef void( tBTA_BLE_MULTI_ADV_CBACK )( tBTA_BLE_MULTI_ADV_EVT event, UINT8 inst_id, void *p_ref,
        tBTA_STATUS status );
...
typedef void( tBTA_DM_BLE_PF_CFG_CBACK )( tBTA_DM_BLE_PF_ACTION action,
        tBTA_DM_BLE_PF_COND_TYPE cfg_cond, tBTA_DM_BLE_PF_AVBL_SPACE avbl_space, tBTA_STATUS status,
...
typedef struct
{
    ...
    BOOLEAN remt_name_not_required; /* Application sets this flag if it already knows the name of
                                       the device */
...
/* Encryption callback*/
typedef void(
        tBTA_DM_ENCRYPT_CBACK )( BD_ADDR bd_addr, tBTA_TRANSPORT transport, tBTA_STATUS result );
...
typedef void( tBTA_BLE_SCAN_REP_CBACK )( tBTA_DM_BLE_REF_VALUE ref_value, UINT8 report_format,
        UINT8 num_records, UINT16 data_len, UINT8 *p_rep_data, tBTA_STATUS status );
...
typedef void( tBTA_BLE_ENERGY_INFO_CBACK )( tBTA_DM_BLE_TX_TIME_MS tx_time,
        tBTA_DM_BLE_RX_TIME_MS rx_time, tBTA_DM_BLE_IDLE_TIME_MS idle_time,
        tBTA_DM_BLE_ENERGY_USED energy_used, tBTA_DM_CONTRL_STATE ctrl_state, tBTA_STATUS status );
...
#define BTA_DM_PM_NO_PREF \
    0x01 /* service has no prefernce on power mode setting. eg. connection to service got closed \
          */
...
**
*******************************************************************************/
BTA_API extern tBTA_STATUS BTA_DmVendorSpecificCommand( UINT16 opcode, UINT8 param_len,
        UINT8 *p_param_buf, tBTA_VENDOR_CMPL_CBACK *p_cback );
...
**
*******************************************************************************/
BTA_API extern void BTA_DmPinReply( BD_ADDR bd_addr, BOOLEAN accept, UINT8 pin_len, UINT8 *p_pin );
...
**
*******************************************************************************/
BTA_API extern void BTA_DmLinkPolicy( BD_ADDR bd_addr, tBTA_DM_LP_MASK policy_mask, BOOLEAN set );
...
**
*******************************************************************************/
BTA_API extern void BTA_DmAddDevice( BD_ADDR bd_addr, DEV_CLASS dev_class, LINK_KEY link_key,
...
**
*******************************************************************************/
BTA_API extern void BTA_DmAddDevWithName( BD_ADDR bd_addr, DEV_CLASS dev_class, BD_NAME bd_name,
        UINT8 *features, LINK_KEY link_key, tBTA_SERVICE_MASK trusted_mask, BOOLEAN is_trusted,
...
**
*******************************************************************************/
BTA_API extern void BTA_DmDiDiscover( BD_ADDR remote_device, tBTA_DISCOVERY_DB *p_db, UINT32 len,
        tBTA_DM_SEARCH_CBACK *p_cback );
...
BTA_API extern tBTA_STATUS BTA_DmGetDiRecord( UINT8 get_record_index,
        tBTA_DI_GET_RECORD *p_device_info, tBTA_DISCOVERY_DB *p_db );
...
**
*******************************************************************************/
BTA_API extern void BTA_DmBleSetBgConnType( tBTA_DM_BLE_CONN_TYPE bg_conn_type,
...
**
*******************************************************************************/
BTA_API extern void BTA_DmSetBlePrefConnParams( BD_ADDR bd_addr, UINT16 min_conn_int,
        UINT16 max_conn_int, UINT16 slave_latency, UINT16 supervision_tout );
...
**
*******************************************************************************/
BTA_API extern void BTA_BleUpdateAdvInstParam( UINT8 inst_id, tBTA_BLE_ADV_PARAMS *p_params );
...
**
*******************************************************************************/
BTA_API extern void BTA_DmEnableScanFilter( UINT8 action, tBTA_DM_BLE_PF_STATUS_CBACK *p_cmpl_cback,
        tBTA_DM_BLE_REF_VALUE ref_value );
...
**
*******************************************************************************/
BTA_API extern void BTA_DmBleScanFilterSetup( UINT8 action, tBTA_DM_BLE_PF_FILT_INDEX filt_index,
