...
#include "sdk/bluedroid/bta/dm/bta_dm_int.h"
#include "sdk/bluedroid/bta/include/bd.h"
#include "sdk/bluedroid/bta/include/bta_api.h"
#include "sdk/bluedroid/bta/include/utl.h"
#include "sdk/bluedroid/bta/sys/bta_sys.h"
#include "sdk/bluedroid/bta/sys/bta_sys_int.h"
...
#include "sdk/bluedroid/vnd/include/vendor_ble.h"
...
**
*******************************************************************************/
void BTA_DmSetVisibility( tBTA_DM_DISC disc_mode, tBTA_DM_CONN conn_mode, UINT8 pairable_mode,
        UINT8 conn_filter )
{
...
{
    APPL_TRACE_API( "BTA_DmSetScanParam: %d, %d, %d, %d", page_scan_interval, page_scan_window,
...
void BTA_DmSetAfhChannels( UINT8 first, UINT8 last )
{
    ...
    if( ( p_msg = (tBTA_DM_API_SET_AFH_CHANNELS_EVT *)GKI_getbuf( sizeof( tBTA_DM_MSG ) ) ) !=
            NULL )
    {
...
void BTA_DmSetAfhChannelAssessment( BOOLEAN enable_or_disable )
{
    ...
    if( ( p_msg = (tBTA_DM_API_SET_AFH_CHANNEL_ASSESSMENT *)GKI_getbuf(
                  sizeof( tBTA_DM_API_SET_AFH_CHANNEL_ASSESSMENT ) ) ) != NULL )
    {
...
**
*******************************************************************************/
tBTA_STATUS BTA_DmVendorSpecificCommand( UINT16 opcode, UINT8 param_len, UINT8 *p_param_buf,
...
**
*******************************************************************************/
void BTA_DmDiscoverUUID( BD_ADDR bd_addr, tSDP_UUID *uuid, tBTA_DM_SEARCH_CBACK *p_cback,
        BOOLEAN sdp_search )
{
    tBTA_DM_API_DISCOVER *p_msg;
...
void BTA_DmBondCancel( BD_ADDR bd_addr )
{
    ...
    if( ( p_msg = (tBTA_DM_API_BOND_CANCEL *)GKI_getbuf( sizeof( tBTA_DM_API_BOND_CANCEL ) ) ) !=
            NULL )
    {
...
**
*******************************************************************************/
void BTA_DmLinkPolicy( BD_ADDR bd_addr, tBTA_DM_LP_MASK policy_mask, BOOLEAN set )
{
    tBTA_DM_API_LINK_POLICY *p_msg;

    if( ( p_msg = (tBTA_DM_API_LINK_POLICY *)GKI_getbuf( sizeof( tBTA_DM_API_LINK_POLICY ) ) ) !=
            NULL )
    {
...
void BTA_DmPasskeyCancel( BD_ADDR bd_addr )
{
    ...
    if( ( p_msg = (tBTA_DM_API_PASKY_CANCEL *)GKI_getbuf( sizeof( tBTA_DM_API_PASKY_CANCEL ) ) ) !=
            NULL )
    {
...
void BTA_DmAddDevice( BD_ADDR bd_addr, DEV_CLASS dev_class, LINK_KEY link_key,
        tBTA_SERVICE_MASK trusted_mask, BOOLEAN is_trusted, UINT8 key_type, tBTA_IO_CAP io_cap )
{
    tBTA_DM_API_ADD_DEVICE *p_msg;

    if( ( p_msg = (tBTA_DM_API_ADD_DEVICE *)GKI_getbuf( sizeof( tBTA_DM_API_ADD_DEVICE ) ) ) !=
            NULL )
    {
...
tBTA_STATUS BTA_DmRemoveDevice( BD_ADDR bd_addr )
{
    ...
    if( ( p_msg = (tBTA_DM_API_REMOVE_DEVICE *)GKI_getbuf(
                  sizeof( tBTA_DM_API_REMOVE_DEVICE ) ) ) != NULL )
    {
...
**
*******************************************************************************/
void BTA_DmAddDevWithName( BD_ADDR bd_addr, DEV_CLASS dev_class, BD_NAME bd_name, UINT8 *features,
        LINK_KEY link_key, tBTA_SERVICE_MASK trusted_mask, BOOLEAN is_trusted, UINT8 key_type,
        tBTA_IO_CAP io_cap )
{
    tBTA_DM_API_ADD_DEVICE *p_msg;
...
void BTA_DmAuthorizeReply( BD_ADDR bd_addr, tBTA_SERVICE_ID service, tBTA_AUTH_RESP response )
{
    ...
    if( ( p_msg = (tBTA_DM_API_AUTH_REPLY *)GKI_getbuf( sizeof( tBTA_DM_API_AUTH_REPLY ) ) ) !=
            NULL )
    {
...
void BTA_DmSignalStrength( tBTA_SIG_STRENGTH_MASK mask, UINT16 period, BOOLEAN start )
{
    ...
    if( ( p_msg = (tBTA_API_DM_SIG_STRENGTH *)GKI_getbuf( sizeof( tBTA_API_DM_SIG_STRENGTH ) ) ) !=
            NULL )
    {
...
void BTA_DmEirAddUUID( tBT_UUID *p_uuid )
{
    ...
    if( ( p_msg = (tBTA_DM_API_UPDATE_EIR_UUID *)GKI_getbuf(
                  sizeof( tBTA_DM_API_UPDATE_EIR_UUID ) ) ) != NULL )
    {
...
void BTA_DmSetEIRConfig( tBTA_DM_EIR_CONF *p_eir_cfg )
{
    ...
    if( ( p_msg = (tBTA_DM_API_SET_EIR_CONFIG *)GKI_getbuf(
                  sizeof( tBTA_DM_API_SET_EIR_CONFIG ) ) ) != NULL )
    {
...
**
*******************************************************************************/
tBTA_STATUS BTA_DmSetLocalDiRecord( tBTA_DI_RECORD *p_device_info, UINT32 *p_handle )
{
...
**
*******************************************************************************/
tBTA_STATUS BTA_DmGetLocalDiRecord( tBTA_DI_GET_RECORD *p_device_info, UINT32 *p_handle )
{
    UINT16 status;
...
**
*******************************************************************************/
void BTA_DmDiDiscover( BD_ADDR remote_device, tBTA_DISCOVERY_DB *p_db, UINT32 len,
        tBTA_DM_SEARCH_CBACK *p_cback )
{
...
void BTA_DmAddBleKey( BD_ADDR bd_addr, tBTA_LE_KEY_VALUE *p_le_key, tBTA_LE_KEY_TYPE key_type )
{
    ...
    if( ( p_msg = (tBTA_DM_API_ADD_BLEKEY *)GKI_getbuf( sizeof( tBTA_DM_API_ADD_BLEKEY ) ) ) !=
            NULL )
    {
...
void BTA_DmAddBleDevice( BD_ADDR bd_addr, tBLE_ADDR_TYPE addr_type, tBT_DEVICE_TYPE dev_type )
{
    ...
    if( ( p_msg = (tBTA_DM_API_ADD_BLE_DEVICE *)GKI_getbuf(
                  sizeof( tBTA_DM_API_ADD_BLE_DEVICE ) ) ) != NULL )
    {
...
void BTA_DmBlePasskeyReply( BD_ADDR bd_addr, BOOLEAN accept, UINT32 passkey )
{
    ...
    if( ( p_msg = (tBTA_DM_API_PASSKEY_REPLY *)GKI_getbuf(
                  sizeof( tBTA_DM_API_PASSKEY_REPLY ) ) ) != NULL )
    {
...
void BTA_DmBleSecurityGrant( BD_ADDR bd_addr, tBTA_DM_BLE_SEC_GRANT res )
{
    ...
    if( ( p_msg = (tBTA_DM_API_BLE_SEC_GRANT *)GKI_getbuf(
                  sizeof( tBTA_DM_API_BLE_SEC_GRANT ) ) ) != NULL )
    {
...
**
*******************************************************************************/
void BTA_DmSetBlePrefConnParams( BD_ADDR bd_addr, UINT16 min_conn_int, UINT16 max_conn_int,
        UINT16 slave_latency, UINT16 supervision_tout )
{
#if BLE_INCLUDED == TRUE
    tBTA_DM_API_BLE_CONN_PARAMS *p_msg;

    if( ( p_msg = (tBTA_DM_API_BLE_CONN_PARAMS *)GKI_getbuf(
                  sizeof( tBTA_DM_API_BLE_CONN_PARAMS ) ) ) != NULL )
    {
...
void BTA_DmSetBleConnScanParams( UINT16 scan_interval, UINT16 scan_window )
{
    ...
    if( ( p_msg = (tBTA_DM_API_BLE_SCAN_PARAMS *)GKI_getbuf(
                  sizeof( tBTA_DM_API_BLE_SCAN_PARAMS ) ) ) != NULL )
    {
...
**
*******************************************************************************/
void BTA_DmSetBleAdvParams( UINT16 adv_int_min, UINT16 adv_int_max, tBLE_BD_ADDR *p_dir_bda )
{
#if BLE_INCLUDED == TRUE
    ...
    if( ( p_msg = (tBTA_DM_API_BLE_ADV_PARAMS *)GKI_getbuf(
                  sizeof( tBTA_DM_API_BLE_ADV_PARAMS ) ) ) != NULL )
    {
        ...
        tBTA_SET_ADV_DATA_CMPL_CBACK *p_adv_data_cback )
{
    ...
    if( ( p_msg = (tBTA_DM_API_SET_ADV_CONFIG *)GKI_getbuf(
                  sizeof( tBTA_DM_API_SET_ADV_CONFIG ) ) ) != NULL )
    {
...
BTA_API extern void BTA_DmBleSetStorageParams( UINT8 batch_scan_full_max,
        UINT8 batch_scan_trunc_max, UINT8 batch_scan_notify_threshold,
        tBTA_BLE_SCAN_SETUP_CBACK *p_setup_cback, tBTA_BLE_SCAN_THRESHOLD_CBACK *p_thres_cback,
        tBTA_BLE_SCAN_REP_CBACK *p_rep_cback, tBTA_DM_BLE_REF_VALUE ref_value )
{
    ...
    if( ( p_msg = (tBTA_DM_API_SET_STORAGE_CONFIG *)GKI_getbuf(
                  sizeof( tBTA_DM_API_SET_STORAGE_CONFIG ) ) ) != NULL )
    {
...
**
*******************************************************************************/
BTA_API extern void BTA_DmBleEnableBatchScan( tBTA_BLE_SCAN_MODE scan_mode, UINT32 scan_interval,
        UINT32 scan_window, tBTA_BLE_DISCARD_RULE discard_rule, tBLE_ADDR_TYPE addr_type,
        tBTA_DM_BLE_REF_VALUE ref_value )
{
    tBTA_DM_API_ENABLE_SCAN *p_msg;

    if( ( p_msg = (tBTA_DM_API_ENABLE_SCAN *)GKI_getbuf( sizeof( tBTA_DM_API_ENABLE_SCAN ) ) ) !=
            NULL )
    {
...
BTA_API extern void BTA_DmBleDisableBatchScan( tBTA_DM_BLE_REF_VALUE ref_value )
{
    ...
    if( ( p_msg = (tBTA_DM_API_DISABLE_SCAN *)GKI_getbuf( sizeof( tBTA_DM_API_DISABLE_SCAN ) ) ) !=
            NULL )
    {
        ...
        tBTA_DM_BLE_REF_VALUE ref_value )
{
    ...
    if( ( p_msg = (tBTA_DM_API_READ_SCAN_REPORTS *)GKI_getbuf(
                  sizeof( tBTA_DM_API_READ_SCAN_REPORTS ) ) ) != NULL )
    {
        ...
        tBTA_BLE_TRACK_ADV_CBACK *p_track_adv_cback )
{
    ...
    if( ( p_msg = (tBTA_DM_API_TRACK_ADVERTISER *)GKI_getbuf(
                  sizeof( tBTA_DM_API_TRACK_ADVERTISER ) ) ) != NULL )
    {
...
BTA_API extern void BTA_DmBleBroadcast( BOOLEAN start )
{
    ...
    if( ( p_msg = (tBTA_DM_API_BLE_OBSERVE *)GKI_getbuf( sizeof( tBTA_DM_API_BLE_OBSERVE ) ) ) !=
            NULL )
    {
        ...
        tBTA_DM_BLE_SEL_CBACK *p_select_cback )
{
    ...
    if( ( p_msg = (tBTA_DM_API_BLE_SET_BG_CONN_TYPE *)GKI_getbuf(
                  sizeof( tBTA_DM_API_BLE_SET_BG_CONN_TYPE ) ) ) != NULL )
    {
...
static void bta_dm_discover_send_msg( BD_ADDR bd_addr, tBTA_SERVICE_MASK_EXT *p_services,
        tBTA_DM_SEARCH_CBACK *p_cback, BOOLEAN sdp_search, tBTA_TRANSPORT transport )
{
    tBTA_DM_API_DISCOVER *p_msg;
    UINT16 len = p_services ?
...
void BTA_DmBleConfigLocalPrivacy( BOOLEAN privacy_enable )
{
    ...
    if( ( p_msg = (tBTA_DM_API_LOCAL_PRIVACY *)GKI_getbuf(
                  sizeof( tBTA_DM_API_ENABLE_PRIVACY ) ) ) != NULL )
    {
...
**
*******************************************************************************/
void BTA_BleEnableAdvInstance( tBTA_BLE_ADV_PARAMS *p_params, tBTA_BLE_MULTI_ADV_CBACK *p_cback,
        void *p_ref )
{
...
**
*******************************************************************************/
void BTA_BleCfgAdvInstData( UINT8 inst_id, BOOLEAN is_scan_rsp, tBTA_BLE_AD_MASK data_mask,
...
void BTA_BleDisableAdvInstance( UINT8 inst_id )
{
    ...
    if( ( p_msg = (tBTA_DM_API_BLE_MULTI_ADV_DISABLE *)GKI_getbuf(
                  sizeof( tBTA_DM_API_BLE_MULTI_ADV_DISABLE ) ) ) != NULL )
    {
...
void BTA_DmBleCfgFilterCondition( tBTA_DM_BLE_SCAN_COND_OP action,
        tBTA_DM_BLE_PF_COND_TYPE cond_type, tBTA_DM_BLE_PF_FILT_INDEX filt_index,
        tBTA_DM_BLE_PF_COND_PARAM *p_cond, tBTA_DM_BLE_PF_CFG_CBACK *p_cmpl_cback,
        tBTA_DM_BLE_REF_VALUE ref_value )
{
    ...
    if( NULL != p_cond )
    {
        ...
        case BTA_DM_BLE_PF_MANU_DATA:
            ...
            len += ( ( p_cond->manu_data.data_len ) * 2 ) + sizeof( UINT16 ) + sizeof( UINT16 ) +
                    sizeof( UINT8 );
            break;
...
void BTA_DmBleScanFilterSetup( UINT8 action, tBTA_DM_BLE_PF_FILT_INDEX filt_index,
        tBTA_DM_BLE_PF_FILT_PARAMS *p_filt_params, tBLE_BD_ADDR *p_target,
        ...
        UINT16 latency, UINT16 timeout )
{
    ...
    if( ( p_msg = (tBTA_DM_API_UPDATE_CONN_PARAM *)GKI_getbuf(
                  sizeof( tBTA_DM_API_UPDATE_CONN_PARAM ) ) ) != NULL )
    {
...
void BTA_DmSetEncryption( BD_ADDR bd_addr, tBTA_TRANSPORT transport,
        tBTA_DM_ENCRYPT_CBACK *p_callback, tBTA_DM_BLE_SEC_ACT sec_act )
{
    tBTA_DM_API_SET_ENCRYPTION *p_msg;

    APPL_TRACE_API( "BTA_DmSetEncryption" ); // todo
    if( ( p_msg = (tBTA_DM_API_SET_ENCRYPTION *)GKI_getbuf(
                  sizeof( tBTA_DM_API_SET_ENCRYPTION ) ) ) != NULL )
    {
...
void BTA_DmCloseACL( BD_ADDR bd_addr, BOOLEAN remove_dev, tBTA_TRANSPORT transport )
{
    ...
    if( ( p_msg = (tBTA_DM_API_REMOVE_ACL *)GKI_getbuf( sizeof( tBTA_DM_API_REMOVE_ACL ) ) ) !=
            NULL )
    {
